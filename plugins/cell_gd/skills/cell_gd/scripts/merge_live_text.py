...
def add_text(root: ET.Element, item: dict, viewbox: tuple[float, float, float, float]) -> ET.Element:
    ...
    if rotation:
        group_attributes["transform"] = f"rotate({rotation:g} {x:g} {y:g})"
    ...
    for index, line in enumerate(lines):
        ...
        if rotation:
            # Rotate the whole multiline label about its original baseline anchor.
            line_attributes.pop("transform", None)
        line_element = ET.SubElement(group, f"{{{SVG_NS}}}text", line_attributes)
        line_element.text = line
    return group


def source_coordinates(item: dict, manifest: dict) -> dict:
    """Map recorded original-image pixels into the unchanged SVG canvas."""
    item = dict(item)
    space = item.get("coordinate_space", "normalized")
    if space not in {"normalized", "pixels", "svg"}:
        raise ValueError("coordinate_space must be normalized, pixels or svg")
    if space == "pixels":
        canvas = manifest.get("source_canvas", {})
        width = finite(canvas.get("width", 0), "source_canvas.width")
        height = finite(canvas.get("height", 0), "source_canvas.height")
        if width <= 0 or height <= 0:
            raise ValueError("pixel coordinates require positive source_canvas width and height")
        item["x"] = finite(item["x"], "x") / width
        item["y"] = finite(item["y"], "y") / height
        item["coordinate_space"] = "normalized"
        if item.get("font_size_space", "pixels") == "pixels":
            item["font_size"] = finite(item.get("font_size", 12), "font_size") / height
            item["font_size_space"] = "normalized"
    return item


def main() -> int:
    parser = argparse.ArgumentParser(description="Merge live text into a Cell_ppt Master SVG.")
    ...
    for item in items:
        ...
        item = source_coordinates(item, manifest)
        element = add_text(root, item, viewbox)
        generated_ids = {node.get("id") for node in element.iter() if node.get("id")}
    ...
    root.set("data-cell-ppt-text-manifest", manifest_path.name)
