...
foreach ($required in @($allocator, $validator, $cacheBuilder, $visibilityCuller, $pptRuntime, $ooxmlRuntime, $runtimeConfigurator)) {
...
& $pythonExe @pythonPrefix $validator --svg $inputPath | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'SVG validation failed.' }
& $pythonExe @pythonPrefix $cacheBuilder --input $inputPath --output-dir $cacheRoot --job-id $baseName | Out-Null
