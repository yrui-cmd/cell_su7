...
Check the expected credit cost before upload. More than 1 credit requires the existing one-time confirmation; do not request confirmation again at download. Credentials remain in Windows DPAPI or macOS Keychain, using standard input for chat-supplied keys.
