...
Confirmed failure in the old PPT route: splitting a canvas-sized white compound path into separate filled contours destroys its holes and creates an opaque white board. Version 0.4 preserves compound contours in native OOXML and rejects legacy split caches. Rebuild such caches from the original SVG; do not try to repair lost holes merely by sending the outer white rectangle to the back.
...
The parser already skips non-rendering display/visibility and transparent paint. `cull_hidden_geometry.py` only collapses adjacent opaque duplicates, keeping the first occurrence; it does not perform occlusion clipping or remove every fully covered path. Cross-layer repeated backgrounds remain in their source positions. Do not delete all hidden, white, background or large objects as a generic remedy. No blanket layer unlock/show/delete operation is part of this workflow.
