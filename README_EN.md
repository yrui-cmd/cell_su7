...
Cell_ppt `v0.1.0` is a Windows Codex skill and plugin that reconstructs scientific figures as editable native Microsoft PowerPoint paths and live text. It plays retained paths from back to front while preserving objects already present on the active slide.
...
- Parse the SVG once, cull fully hidden and duplicate playback paths, retain partially visible paths, and create each retained object exactly once.
...
The repository freezes local workflow and playback behavior. Built-in Image 2 and the remote vector service are nondeterministic, so newly generated geometry may vary slightly across runs.
