...
if ($plugin.name -ne 'cell_su7' -or $plugin.version -ne '0.4.1' -or $plugin.skills -ne './skills/' -or $plugin.license -ne 'MIT') { throw 'Plugin identity is invalid.' }
...
Write-Output 'PACKAGE_OK|skill=cell_su7|version=0.4.1|platforms=windows,macos|freeze=removed|secret_scan=clean'
